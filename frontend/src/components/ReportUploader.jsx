    ...
    <div style={{ maxWidth: '900px', margin: '0 auto', width: '100%' }}>
      {!analysis ? (
        <div className="card">
          ...
          >
            <input type="file" ref={fileInputRef} style={{ display: 'none' }} onChange={(e) => e.target.files?.[0] && setFile(e.target.files[0])} accept="image/*,.pdf, .docs" />
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          <div className="card">
            ...
            <div style={{ fontSize: '0.93rem', lineHeight: 1.7, color: 'var(--text-secondary)' }}>
              ...
              <div style={{ flex: 1, overflowY: 'auto', padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                {chatMessages.map((msg, i) => (
                  <div key={i} style={{ display: 'flex', justifyContent: msg.role === 'user' ? 'flex-end' : 'flex-start' }}>
                    ...
                    }}>{msg.role === 'assistant' ? <MarkdownBlock content={msg.content} /> : msg.content}</div>
                  </div>
                ))}
                {chatLoading && (
